...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof midrangeabs, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 1', function test( t ) {
	t.strictEqual( midrangeabs.length, 1, 'has expected arity' );
	t.end();
});

tape( 'the function calculates the mid-range of absolute values of a one-dimensional ndarray', function test( t ) {
	var x;
	var v;

	x = [ 1.0, -2.0, -4.0, 5.0, 0.0, 3.0 ];
	v = midrangeabs( [ vector( x, 6, 1, 0 ) ] );
	t.strictEqual( v, 2.5, 'returns expected value' );

	x = [ -4.0, -5.0 ];
	v = midrangeabs( [ vector( x, 2, 1, 0 ) ] );
	t.strictEqual( v, 4.5, 'returns expected value' );

	x = [ -0.0, 0.0, -0.0 ];
	v = midrangeabs( [ vector( x, 3, 1, 0 ) ] );
	t.strictEqual( isPositiveZero( v ), true, 'returns expected value' );

	x = [ NaN ];
	v = midrangeabs( [ vector( x, 1, 1, 0 ) ] );
